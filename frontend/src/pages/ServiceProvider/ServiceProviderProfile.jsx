...
function ServiceProviderProfile() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_BASE_URL}/api/users/me`, {
      ...
      const desig = res.data.designation || '';
      ...
      const existing = localStorage.getItem(key) || localStorage.getItem('agrochain-user-avatar-service-provider') || localStorage.getItem('agrochain-user-avatar') || '';
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (avatarPreview) {
        const key = designation ? `agrochain-user-avatar-service-provider-${designation.toLowerCase().replace(/\s+/g, '-')}` : 'agrochain-user-avatar-service-provider';
    ...
    <>
      ...
      <div className="container py-4">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="d-flex align-items-center mb-3" style={{ gap: 10 }}>
          ...
          <h3 className="fw-bold mb-0" style={{ color: '#2C5F2D' }}>Service Provider Profile</h3>
        </motion.div>
        {loading ? (
