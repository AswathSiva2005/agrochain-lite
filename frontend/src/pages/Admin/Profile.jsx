...
function AdminProfile() {
  ...
  const [avatarPreview, setAvatarPreview] = useState(localStorage.getItem('agrochain-user-avatar-admin') || localStorage.getItem('agrochain-user-avatar') || '');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (avatarPreview) {
        localStorage.setItem('agrochain-user-avatar-admin', avatarPreview);
      }
      alert('Profile saved');
    } catch (err) {
  ...
  const containerVariants = { hidden: { opacity: 0, y: 16 }, show: { opacity: 1, y: 0, transition: { duration: 0.5 } } };
    ...
    <>
      ...
      <div className="container mt-4">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="d-flex align-items-center mb-3" style={{ gap: 10 }}>
          ...
          <h3 className="fw-bold mb-0" style={{ color: '#2C5F2D' }}>Admin Profile</h3>
        </motion.div>
        {loading ? (
          <div className="text-center p-4"><div className="spinner-border text-success" role="status" /></div>
        ) : (
              ...
              <motion.div variants={containerVariants} initial="hidden" animate="show" className="card border-0 shadow-lg" style={{ borderRadius: 16 }}>
                <div className="card-body text-center">
                  <div className="mb-3 position-relative d-inline-block">
                    <img src={avatarPreview || 'https://via.placeholder.com/160?text=Avatar'} alt="Avatar" style={{ width: 160, height: 160, borderRadius: '50%', objectFit: 'cover', border: '3px solid #2C5F2D' }} />
                    <label className="btn btn-success position-absolute" style={{ right: -6, bottom: -6, borderRadius: '50%', width: 44, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <FaCamera />
                      <input type="file" accept="image/*" hidden onChange={(e) => { const file = e.target.files?.[0]; setAvatarFile(file || null); if (file) { const reader = new FileReader(); reader.onload = () => setAvatarPreview(reader.result); reader.readAsDataURL(file); } }} />
                    </label>
                  </div>
