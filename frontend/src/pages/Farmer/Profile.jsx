...
function FarmerProfile() {
  ...
  const [avatarPreview, setAvatarPreview] = useState(localStorage.getItem('agrochain-user-avatar-farmer') || localStorage.getItem('agrochain-user-avatar') || '');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (avatarPreview) {
        localStorage.setItem('agrochain-user-avatar-farmer', avatarPreview);
      }
      alert('Profile saved');
    } catch (err) {
    ...
    <>
      ...
      <div className="container py-4">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="d-flex align-items-center mb-3" style={{ gap: 10 }}>
          ...
          <h3 className="fw-bold mb-0" style={{ color: '#2C5F2D' }}>Farmer Profile</h3>
        </motion.div>
        {loading ? (
